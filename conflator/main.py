...
def create_points_dict(roads_df):
    ...
    with open('/conflator-cache/dup_points.geojson', 'w') as f:
...
def create_ball_tree(leaf_size=40):
    print(os.listdir('/conflator-cache'))
    if os.path.exists('/conflator-cache/ball_tree.pickle') and os.path.exists('/conflator-cache/ball_tree_ids.pickle') and USE_CACHE:
        with open('/conflator-cache/ball_tree.pickle', 'rb') as f:
            tree = pickle.load(f)
        with open('/conflator-cache/ball_tree_ids.pickle', 'rb') as f:
            ids = pickle.load(f)
        return tree, ids
    else:
        ...
        with open('/conflator-cache/ball_tree.pickle', 'wb') as f:
            pickle.dump(tree, f)
        with open('/conflator-cache/ball_tree_ids.pickle', 'wb') as f:
            pickle.dump(ids, f)
        return tree, ids


def load_mds_from_file():
    df = pd.read_csv('/cache/mds-trips.csv')
    # load geom column as wkt to geodataframe
    df['geom'] = df['geom'].progress_apply(lambda wkt: shapely.wkt.loads(wkt))
    return df

    with open('/cache/mds.json') as f:
        data = json.load(f)
...
def main():
    ...
    mds_df.drop(columns=['distance', 'distance_meters', 'geom_rad', 'geom', 'idx']).to_csv('/cache/mds-trips-with-roadsegid.csv', index=False)
